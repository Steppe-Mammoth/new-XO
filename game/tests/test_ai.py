...
def test_ai_all_result_for_first_step():
    t = build_table()
    ai_all_step = AI_FINDER._find_best_step(symbol=p_ai.symbol, table=t.game_table, combinations=t.combinations)
    ai_get_step = AI_FINDER.get_step(symbol=p_ai.symbol, table=t.game_table, combinations=t.combinations)
...
def test_ai_one_player_win_3():
    t = build_table()
    for i in range(3):
        i_row, i_column = AI_FINDER.get_step(combinations=t.combinations, symbol=p_ai.symbol, table=t.game_table)
        t.set_symbol_cell(index_row=i_row, index_column=i_column, symbol=p_ai.symbol)

    result = CHECKER.result_player(symbol=p_ai.symbol, table=t.game_table, combinations=t.combinations)
    print(result)
...
def test_ai_battle_1_fail_to_win_enemy_3():  # Перебити гравця не давши зібрати 3/3
    ...
    i_row, i_column = AI_FINDER.get_step(symbol=p_ai.symbol, table=t.game_table, combinations=t.combinations)
    t.set_symbol_cell(index_row=i_row, index_column=i_column, symbol=p_ai.symbol)  #

    assert t.game_table[1][1].symbol == p_ai.symbol
