...
from game import TableParam
from game.core.cheker import CheckerBase, CheckerDefault
from game.core.game_xo import GameBase, Game
from game.core.players.player import PlayerBase, Player
from game.core.players.players import PlayersBase, Players
from game.core.result import GameStateBase, GameState, ResultCode
from game.core.table.table import TableBase, TableDefault
from game.exceptions.core_exceptions import CellAlreadyUsedError, PlayersInstanceError, CheckerError, TableInstanceError
from game.tests.builder import build_table, build_player_list

PLAYER: Type[PlayerBase] = Player
PLAYERS: Type[PlayersBase] = Players
TABLE: Type[TableBase] = TableDefault
...
def test_bad_verify_players_instance_error_game():
    ...
    for ps in players_list:
        with pytest.raises(PlayersInstanceError):
            GAME(players=ps, table=TableDefault(param))
...
def test_bad_verify_table_instance_error_game():
    ...
    param = TableParam(3, 3, 3)
    table_list = [param, TableDefault, TableBase, 'fdfd', 34343]
    for t in table_list:
