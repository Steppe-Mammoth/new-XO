...
from game.core.table.combinations import CombDefault, verify_combs_list
...
ClearGameFieldType = tuple[list[None], ...]
GameFieldType = tuple[list[Cell | None], ...]
TableT = TypeVar('TableT', bound='TableBase', covariant=True)


def create_2d_list(row: int, column: int) -> ClearGameFieldType:
...
class TableDefault(TableBase):
    ...
    def __init__(self, param: TableParam, combinations_list: Optional[CombsType] = None):
        """
        :param param: Instance class TableParam
        :param combinations_list: The list of winning combinations is used to
          determine the winning cells for the players.
        ...
        if not combinations_list:
            combinations_list: CombsType = CombDefault.get_combinations(size_row=param.ROW,
